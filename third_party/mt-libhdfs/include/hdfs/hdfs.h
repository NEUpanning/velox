...
/**
 * hdfsConnect - Connect to a hdfs file system with DelegationToken.
 * Connect to the hdfs.
 * @param nn   The NameNode.  See hdfsBuilderSetNameNode for details.
 * @param port The port on which the server is listening.
 ...
 * @deprecated Use hdfsBuilderConnect instead.
 */
hdfsFS hdfsConnectWithToken(const char * host, tPort port, const char * token);
...
enum HdfsLogLevel {
   Log_FATAL = 0, Log_ERROR, Log_WARNING, Log_INFO, Log_DEBUG1, Log_DEBUG2, Log_DEBUG3
};

/**
 * Set the HDFS RootLogger output fd.
 *
 * The fd will be auto closed, when the RootLogger destruct.
 *
 * @param fd The opened writable fd
 */
void setLogOutputFd(int fd);

/**
 * Print log to the HDFS RootLogger.
 *
 * @param level The HdfsLogLevel
 * @param fmt The log message format of variable-argument
 */
void hdfsLogPrintf(enum HdfsLogLevel level, const char * fmt, ...);
